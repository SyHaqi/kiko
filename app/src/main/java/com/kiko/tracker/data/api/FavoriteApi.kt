 ...
 *   ->   {"created_at": 1789620248} on success, plain 200 with no interesting body otherwise
 *
...
class FavoriteApi(context: Context) {
    ...
    suspend fun setFavorited(kind: FavoriteKind, malId: Int, favorited: Boolean): Unit = withContext(Dispatchers.IO) {
        ...
        client.newCall(request).execute().use { resp ->
            ...
            if (!resp.isSuccessful) throw IOException("Favorite toggle failed (${resp.code}): ${text.take(300)}")
        }
    }
}